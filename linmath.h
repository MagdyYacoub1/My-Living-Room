...
static void mat4x4_printf(mat4x4 M)
{
	int i, j;
	for (i=0; i<4; i++) {
		for (j=0; j<4; j++) {
...
static void vec4_set(vec3 v, float x, float y, float z, float w)
{
	v[0] = x;
	v[1] = y;
	v[2] = z;
...
static void vec3_set_arr(vec3 v, float *arr)
{
	v[0] = arr[0];
	v[1] = arr[1];
	v[2] = arr[2];
...
static void vec3_printf(vec3 v)
{
	printf("%f, %f, %f\n", v[0], v[1], v[2]);
}
